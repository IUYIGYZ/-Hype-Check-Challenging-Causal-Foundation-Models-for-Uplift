...
实现目录：`项目/baseline_benchmark/`。CausalPFN 已作为可选的基础模型接入：它对相同测试 ID 输出 `cate_pred`，并使用与其他模型相同的评估代码公平比较。
...
`causalpfn` 可以和已有模型名一起传入；其预测仍写入相同的
`predictions.parquet`，并在 `metrics.csv` 中使用相同的 Qini/AUUC/uplift 指标。
...
- CausalPFN 已通过 `baseline_benchmark/models.py` 中的 `CausalPFNEstimator` 接入；依赖保持可选，因为首次使用可能下载预训练权重。
